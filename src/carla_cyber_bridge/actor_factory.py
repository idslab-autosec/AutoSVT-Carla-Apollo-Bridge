...
class actor_update_thread(Thread):
    def __init__(self, node, actor, frame_id, timestamp):
    ...
    def run(self):
        try:
            self.actor.update(self.frame_id, self.timestamp)
        except RuntimeError as e:
            self.node.logwarn("Update actor {} failed: {}".format(
                self.actor.__class__.__name__, e))
...
class ActorFactory(object):
    ...
    def update_actor_states(self, frame_id, timestamp):
        ...
        # Single Thread
        # logging.info("before self.lock")
        # with self.lock:
        #     logging.info("into self.lock")
        #     for actor_id in self.actors:
        #         logging.info("actor_id %s",actor_id)
        #         try:
        #             logging.info("before %s update",self.actors[actor_id])
        ...
        with self.lock:
            for actor_id in self.actors:
                tmp_thread = actor_update_thread(
                    self.node, 
    ...
    def _destroy_object(self, actor_id, delete_actor):
        ...
        if carla_actor and delete_actor:
            carla_actor.destroy()
        logging.info("Removed %s(id=%s)", actor.__class__.__name__, actor.uid)
    ...
    def _create_object(self, uid, type_id, name, attach_to, spawn_pose, carla_actor=None):
        ...
        logging.info("Created %s(id=%s)", actor.__class__.__name__, actor.uid)
